...
const STEP_BORDER = { 0: "#bfdbfe", 1: "#c4b5fd", 2: "#fde68a" };
...
export default function LanguageTrainer() {
  ...
  const removeSet = (id) => {
    ...
    setTrashedSets(t => [...t, { set, pairs: sp, trashedAt: Date.now() }]);
  ...
  const restoreTrashedSet = (idx) => {
    ...
    setTrashedSets(t => t.filter((_, i) => i !== idx));
  };
  ...
  // Get the step (mastery level) for a given pair
  ...
  const startTraining = () => {
    ...
    // Initialize progress per word based on its own mastery
    const init = {};
  ...
  const buildNextQueue = (prog, compl) => {
    ...
    if (remaining.length === 0) {
      // Update mastery for each completed word
      const newMastery = { ...mastery };
      const toArchive = [];
  ...
  const checkAnswer = () => {
    ...
    if (wordStep === 0) {
      // Beginner: phase 1 = type English, phase 2 = type Spanish
      if (p.phase === 1) {
      ...
      // Professional / Legend: just type Spanish
      correct = answer.trim().toLowerCase() === pair.y.toLowerCase();
      correctAns = pair.y;
    }
  ...
  // Progress bar: accounts for mixed mastery levels
  ...
  if (mode === "done") {
    ...
    return (
      <div style={{ maxWidth: 520, margin: "0 auto", padding: 24, fontFamily: "system-ui, -apple-system, sans-serif" }}>
        <div style={{ textAlign: "center", padding: "48px 0" }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🎉</div>
          <h2 style={{ color: "#111", margin: "0 0 8px" }}>Training complete!</h2>
          <p style={{ color: "#666", margin: "0 0 12px" }}>
            You nailed all {trainingPairs.length} pairs.
          </p>
          {newlyMastered > 0 && (
          ...
          <button onClick={() => { setMode("input"); setProgress({}); setCompleted([]); setSelectedSetIds([]); }}
            style={{ padding: "10px 24px", fontSize: 15, background: "#8b5cf6", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer" }}>
            Back to word list
          </button>
        </div>
      </div>
  ...
  if (mode === "training") {
    ...
    const bgGrad = STEP_BG[wordStep];
    const borderColor = STEP_BORDER[wordStep];
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: 24, fontFamily: "system-ui, -apple-system, sans-serif" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <button onClick={() => setMode("input")}
            style={{ background: "none", border: "none", cursor: "pointer", color: "#666", fontSize: 14 }}>← Back</button>
          <span style={{ fontSize: 13, color: "#666" }}>✅ {completedCount}/{trainingPairs.length}</span>
        </div>
        <div style={{ height: 6, background: "#e5e7eb", borderRadius: 3, marginBottom: 32, overflow: "hidden" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ fontSize: 28, fontWeight: 600, color: "#111", marginTop: 16 }}>{prompt}</div>
        </div>
        <input ref={inputRef} value={answer} onChange={e => setAnswer(e.target.value)} onKeyDown={handleKeyDown}
          ...
          style={{
            ...
            border: feedback === "correct" ? "2px solid #22c55e" : feedback === "wrong" ? "2px solid #ef4444" : "1px solid #d1d5db",
            outline: "none", background: feedback === "correct" ? "#f0fdf4" : feedback === "wrong" ? "#fef2f2" : "#fff"
          }} />
        {feedback === "wrong" && (
          <div style={{ marginTop: 12, padding: 12, background: "#fef2f2", borderRadius: 8, border: "1px solid #fecaca" }}>
            ...
            <div style={{ color: "#111", fontSize: 18, fontWeight: 500, marginTop: 4 }}>{correctAnswerCache}</div>
  ...
  return (
    <div style={{ maxWidth: 520, margin: "0 auto", padding: 24, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <TrixbiLogo />
      <p style={{ color: "#666", margin: "0 0 24px", fontSize: 14 }}>Add word pairs to sets, then train.</p>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>English</div>
            <input ref={xInputRef} value={currentX} onChange={e => setCurrentX(e.target.value)} onKeyDown={handleAddKeyDown}
              placeholder="Word / Phrase"
              style={{ width: "100%", padding: "10px 12px", fontSize: 14, border: "1px solid #d1d5db", borderRadius: 8, outline: "none", boxSizing: "border-box" }} />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, color: "#888", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>Spanish</div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <select value={currentSetId} onChange={e => setCurrentSetId(e.target.value)}
            style={{ flex: 1, padding: "8px 10px", fontSize: 13, border: "1px solid #d1d5db", borderRadius: 8, outline: "none", background: "#fff" }}>
            {sets.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          <button onClick={addPair} style={{
            padding: "8px 18px", fontSize: 15, background: "#2563eb", color: "#fff",
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        ...
        <button onClick={addSet} style={{
          padding: "8px 14px", fontSize: 13, background: "#f3f4f6", color: "#333",
      ...
      {selectedSetIds.length > 0 && (
        <button onClick={() => setSelectedSetIds([])}
          style={{
            ...
            background: "transparent", color: "#8b5cf6", border: "1px solid #c4b5fd",
      ...
      {sets.map(set => {
        ...
        return (
          <div key={set.id}
            ...
            style={{
              border: isDragOver ? "2px dashed #8b5cf6" : isSelected ? "1.5px solid #c4b5fd" : "1px solid #e5e7eb",
              borderRadius: 10, marginBottom: 12, overflow: "hidden",
              background: isDragOver ? "#f5f3ff" : isSelected ? "#f5f3ff" : "#fff", transition: "all 0.15s ease"
            }}>
            <div style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "10px 14px", background: isSelected ? "#ede9fe" : "#f9fafb", cursor: "pointer"
            }} onClick={() => toggleExpandSet(set.id)}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ transform: isExpanded ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.2s", display: "inline-block", fontSize: 10, color: "#999" }}>▶</span>
                ...
                {editingSetId === set.id ? (
                  <input value={editingSetName} onChange={e => setEditingSetName(e.target.value)}
                    ...
                    style={{ padding: "2px 6px", fontSize: 14, border: "1px solid #bbb", borderRadius: 4, outline: "none", width: 120 }} />
                ) : (
                  <span style={{ fontWeight: 600, fontSize: 14, color: "#111" }}>{set.name}</span>
                )}
                <span style={{ fontSize: 12, color: "#999" }}>({sp.length})</span>
              </div>
            ...
            {isExpanded && sp.map(p => (
              <div key={p.id} draggable onDragStart={() => handleDragStart(p.id)}
                style={{
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  padding: "8px 14px", borderBottom: "1px solid #f3f4f6", fontSize: 14, cursor: "grab", userSelect: "none"
                }}>
                <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                  <span style={{ color: "#ccc", marginRight: 4, fontSize: 12 }}>⠿</span>
                  <span style={{ fontWeight: 500, color: "#111" }}>{p.x}</span>
                  <span style={{ color: "#999" }}>→</span>
                  <span style={{ color: "#555" }}>{p.y}</span>
                </div>
            ...
            {isExpanded && sp.length === 0 && (
              <div style={{ padding: "12px 14px", color: "#bbb", fontSize: 13, textAlign: "center" }}>
      ...
      <button onClick={startTraining} disabled={eligibleCount === 0}
        style={{
          ...
          background: eligibleCount === 0 ? "#d1d5db" : "#2563eb", color: "#fff",
      ...
      <div style={{ marginTop: 32, borderTop: "1px solid #e5e7eb", paddingTop: 20 }}>
        <button onClick={() => setShowLearned(!showLearned)}
          style={{
            ...
            color: "#666", display: "flex", alignItems: "center", gap: 6, padding: 0
          }}>
          ...
          <div style={{ marginTop: 12 }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
              <button onClick={selectAllLearned} style={{
                padding: "4px 10px", fontSize: 12, background: "#f3f4f6", color: "#333",
              ...
              {learnedSelected.length > 0 && (
                <>
                  <select value={moveToSetId} onChange={e => setMoveToSetId(e.target.value)}
                    style={{ padding: "4px 8px", fontSize: 12, border: "1px solid #d1d5db", borderRadius: 6, background: "#fff" }}>
                    <option value="">Move to set...</option>
                    {sets.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <button onClick={restoreBulk} disabled={!moveToSetId}
                    style={{
                      padding: "4px 10px", fontSize: 12, background: moveToSetId ? "#2563eb" : "#d1d5db",
            ...
            <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, overflow: "hidden" }}>
              {learned.map(p => {
                ...
                return (
                  <div key={p.id} style={{
                    display: "flex", justifyContent: "space-between", alignItems: "center",
                    padding: "8px 14px", borderBottom: "1px solid #f3f4f6", fontSize: 14,
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      <span style={{ color: "#999" }}>→</span>
                      <span style={{ color: "#555" }}>{p.y}</span>
                      {fromSet && <span style={{ fontSize: 11, color: "#bbb", marginLeft: 4 }}>from {fromSet.name}</span>}
                    </div>
                    <div style={{ display: "flex", gap: 4 }}>
                      <button onClick={() => restoreSingle(p.id)}
                        style={{ padding: "3px 8px", fontSize: 11, background: "#f0fdf4", color: "#16a34a",
      ...
      {trashedSets.length > 0 && (
        <div style={{ marginTop: 20, borderTop: "1px solid #e5e7eb", paddingTop: 16 }}>
          <button onClick={() => setShowTrashedSets(!showTrashedSets)}
            style={{
              background: "none", border: "none", cursor: "pointer", fontSize: 15, fontWeight: 600,
              color: "#999", display: "flex", alignItems: "center", gap: 6, padding: 0
            }}>
          ...
          {showTrashedSets && (
            <div style={{ marginTop: 10 }}>
              {trashedSets.map((t, idx) => (
                <div key={idx} style={{
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  padding: "10px 14px", border: "1px solid #f3f4f6", borderRadius: 8, marginBottom: 8,
                  background: "#fafafa"
                }}>
                  <div>
                    <span style={{ fontWeight: 600, fontSize: 14, color: "#666" }}>{t.set.name}</span>
                    <span style={{ fontSize: 12, color: "#bbb", marginLeft: 6 }}>({t.pairs.length} word{t.pairs.length !== 1 ? "s" : ""})</span>
                  </div>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button onClick={() => restoreTrashedSet(idx)}
                      style={{ padding: "4px 10px", fontSize: 12, background: "#f0fdf4", color: "#16a34a",
                        border: "1px solid #bbf7d0", borderRadius: 5, cursor: "pointer" }}>
                      ↩ Restore
                    </button>
                    <button onClick={() => deleteTrashedSetPermanently(idx)}
                      style={{ padding: "4px 10px", fontSize: 12, background: "#fef2f2", color: "#dc2626",
